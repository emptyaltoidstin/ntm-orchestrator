# ntm-orchestrator v2.3 – Expected Behavior
...
7. **Secure runtime dir**: marker/state files live under a private runtime directory (mode `0700`), not world-readable shared paths.
8. **Exact-path cleanup only**: hooks delete only specific owned files inside the runtime dir; no wildcard cleanup.

## Stop/abandon guard

If `<runtime>/active-session.json` (global index) exists, the Stop hook **reconciles against tmux** before blocking. Session-scoped state lives in `<runtime>/<session>/state.json`.
