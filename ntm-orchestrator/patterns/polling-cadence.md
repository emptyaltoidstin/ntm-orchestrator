...
if [ "$current_terse" != "$LAST_TERSE" ]; then
  ...
  active_count=$(jq '.sessions[0].active_count' /tmp/ntm-orch-<session>-status.json)
  error_count=$(jq '.sessions[0].error_count' /tmp/ntm-orch-<session>-status.json)
  completion_pct=$(jq '.sessions[0].completion_pct' /tmp/ntm-orch-<session>-status.json)
...
| 10–30 min | 180s | `--robot-terse` | `--robot-status` + `--robot-tail` on anomaly | Steady state |
...
| `--robot-tail` (30 lines) | ~800 | On anomaly |
...
When `--robot-status` JSON shows anomalies:

| Condition | Action |
|-----------|--------|
| `error_count > 0` | Fetch `--robot-health=<session>` for details |
| `idle_count > 0` with active tasks | Fetch `--robot-tail` to check for stalls |
