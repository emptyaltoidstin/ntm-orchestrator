...
3. **No inline mega-prompts.** If a prompt exceeds 2000 characters, write to `/tmp/ntm-orch-<session>-pane<N>.md` and use `--file` (ntm send) or `--msg-file` (robot-send).
...
8. **Temp files go to `/tmp/ntm-orch-<session>-*`** — never pollute the project tree.
...
| Send with context | `ntm send <session> --pane=N --file=/path -c file1 -c file2 --json` |
...
| Agent needs file context | Send with `-c` flag via `ntm send` |
...
- `/tmp/ntm-orch-<session>-manifest.json`
...
**Operational note:** The PreToolUse hook writes `/tmp/ntm-orch-<session>-state.json` (session-scoped) and `/tmp/ntm-orch-__global__-active-session.json` (global index) on successful `--robot-spawn`. On `ntm kill`, both are cleared. The Stop hook reads the global index to prevent accidental exit while a session is active.

**Temp file naming invariant:** No new `/tmp/ntm-orch-*` files may be introduced unless they include `<session>` or `__global__` in the filename. This ensures Phase 6 cleanup (`rm -f /tmp/ntm-orch-<session>-*`) is truthful.
...
Fill variables: `{{task_id}}`, `{{task_description}}`, `{{file_scope}}`, `{{acceptance_criteria}}`, `{{pane_name}}`, `{{project_slug}}`, `{{quality_gates}}`.
...
Write to `/tmp/ntm-orch-<session>-pane<N>.md`.
...
```bash
ntm send <session> --pane=<N> --file=/tmp/ntm-orch-<session>-pane<N>.md --json
```

With context files:
```bash
ntm send <session> --pane=<N> --file=/tmp/ntm-orch-<session>-pane<N>.md -c src/auth.ts -c src/types.ts --json
```
...
### Core Principle: Terse as Change Detector

**`--robot-terse` is a cheap change detector, NOT an authoritative state source.**

Its format may change between NTM versions. Never parse it for structured data.

**Strategy:**
1. Poll `--robot-terse` (cheap, ~100 tokens)
2. Compare to `LAST_TERSE`
3. If different → fetch `--robot-status` (JSON, authoritative)
4. Update state from JSON fields only
5. Store new terse in `LAST_TERSE`

This makes the skill robust against terse format changes.
...
|-----------|----------|------------------|----------------------------------|
| 0–2 min   | No poll  | —                | —                                |
| 2–10 min  | 120s     | `--robot-terse`  | `--robot-status` if changed      |
| 10–30 min | 180s     | `--robot-terse`  | `--robot-status` + `--robot-tail` on anomaly |
...
2. **Cheap change detection:**
   ...
   if [ "$current_terse" != "$LAST_TERSE" ]; then
     ntm --robot-status > /tmp/ntm-orch-<session>-status.json
     # Parse JSON for authoritative state
   fi
   LAST_TERSE="$current_terse"
   ```

3. **Interpret status from JSON:**
   - `error_count > 0` → `--robot-health=<session>`
   - `idle_count > 0` with active tasks → check `--robot-tail`
   - `completion_pct == 100` → Phase 4

4. **Inbox check:** `fetch_inbox(project_key, agent_name="Orchestrator")`

5. **Update state tracking**

6. **Check escalation triggers**

7. **Calculate next poll time**
...
| FILE_RESERVATION_CONFLICT | Arbitrate: earlier assignment wins |
| Agent crash | Auto-restart handles; escalate after 3+ |
| Agent stall (>10 min idle) | Nudge, check output, consider refresh |
...
| Git conflict | Instruct rebase; provide commands if stuck |
...
```bash
ntm --robot-tail=<session> --panes=<N> --lines=50 | grep -E "(typecheck|lint|test).*(pass|success|✓)"
```
...
Clean up: `rm -f /tmp/ntm-orch-<session>-*`
