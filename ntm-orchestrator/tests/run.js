...
// Known test session names from scenarios.yaml + global prefix.
// Only remove files for these sessions to avoid clobbering an in-flight real session.
const TEST_SESSION_PREFIXES = [
  'ntm-orch-__global__-',
  'ntm-orch-foo-',
  'ntm-orch-bar-',
  'ntm-orch-trial01-',
  'ntm-orch-dead-session-',
  'ntm-orch-emptysess-',
  'ntm-orch-new-session-',
  'ntm-orch-unknown-',
];
...
function resetTmpState() {
  // Surgically remove only files belonging to known test sessions.
  try {
    const tmpFiles = fs.readdirSync('/tmp/');
    for (const f of tmpFiles) {
      if (TEST_SESSION_PREFIXES.some(prefix => f.startsWith(prefix))) {
        rmIfExists(`/tmp/${f}`);
      }
    }
  } catch {
