...
```bash
ntm send mysession --pane=2 --file=/tmp/prompt-2.md --json
sleep 2
ntm send mysession --pane=3 --file=/tmp/prompt-3.md --json
```
...
if [ "$current" != "$LAST_TERSE" ]; then
  ...
  ntm --robot-status > /tmp/status.json
fi
LAST_TERSE="$current"
```
