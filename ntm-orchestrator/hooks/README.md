...
## Design Note: Capture-Before-Kill Enforcement (v2.2.2+)
...
1. When `ntm save <session>` runs, the hook writes a **save marker**: `/tmp/ntm-orch-<session>-saved.json`
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/skills/ntm-orchestrator-v2.2.2/hooks/pre-tool-use.js",
            "timeout": 5
          }
        ]
      }
    ...
    "SubagentStop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/skills/ntm-orchestrator-v2.2.2/hooks/subagent-stop.js",
            "timeout": 5
          }
        ]
      }
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/skills/ntm-orchestrator-v2.2.2/hooks/stop.js",
            "timeout": 5
          }
        ]
      }
...
- `ntm kill <session>` without a recent save (checks for save marker at `/tmp/ntm-orch-<session>-saved.json`)
...
echo '{"tool_name":"Bash","tool_input":{"command":"ntm save test-session -o /tmp/output"}}' | node hooks/pre-tool-use.js
echo $?  # expect 0
cat /tmp/ntm-orch-test-session-saved.json  # marker should exist
...
Prevents the orchestrator from exiting while an NTM session is still active. Checks for `/tmp/ntm-orch-__global__-active-session.json` (global index written by pre-tool-use.js on spawn, cleared on kill). Session-scoped state is stored in `/tmp/ntm-orch-<session>-state.json`.
...
The hooks use session-scoped marker files in `/tmp/` to track state:

| File | Written by | Purpose |
|------|-----------|---------|
| `/tmp/ntm-orch-<session>-state.json` | `--robot-spawn` | Tracks session spawn time, PID |
| `/tmp/ntm-orch-<session>-saved.json` | `ntm save` | Proves output was captured before kill |
| `/tmp/ntm-orch-<session>-hook-last-poll.json` | Poll commands | Rate-limits polling frequency |
| `/tmp/ntm-orch-__global__-active-session.json` | `--robot-spawn` | Global index for stop.js |
...
```json
{
  ...
  "save_attempted": true,
  "save_succeeded": true,
  "command": "ntm save my-session -o /tmp/output"
}
```
...
All session-scoped files are cleaned up when `ntm kill <session>` is allowed to proceed. The naming convention ensures `rm -f /tmp/ntm-orch-<session>-*` in Phase 6 teardown is safe and complete.
