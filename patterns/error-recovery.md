...
2. **Repeated crashes (2x same pane):**
   ...
   ntm send <session> --pane=<N> "Retry with: <specific fix>" --json
   ```
...
1. **Shared-tree worker conflict:**
   - Instruct worker to stop edits and report exact conflicting files.
   - Reassign integration/conflict resolution to Integrator.
...
3. **Stuck on conflict:**
   ...
   ntm send <session> --pane=<N> --file=<runtime>/<session>/conflict-help-<N>.md --json
   ```
...
1. **First failure:**
   ```bash
   ntm send <session> --pane=<N> "Quality gates required before completion. Run:
   {{quality_gates}}
   
   Fix any failures before reporting complete." --json
   ```
...
3. **Hard rule:** Conflicted worker must stop editing until reassigned.

4. **Notify losing agent:**
   ```javascript
   send_message({
     project_key: 'slug',
...
5. **Options for conflicted/waiting agent:**
